...
window.onload = async function () {
    ...
    // Create a promise that shows the overlay after 1 second
    ...
    try {
        ...
        const initializationPromise = (async () => {
            ...
            if (currentConversation == null) {
                ...
                if (currentConversationTouched == "true") {
            ...
            // Fetch and set default settings
        ...
        // Clear the timeout and remove the overlay if it was shown
...
function getDefaultConversationName() {
    ...
    if (firstMessage) {
        ...
        Name = firstMessage.content.substring(0, 45) + (firstMessage.content.length > 45 ? '...' : '');
        Name = Name.replace("'", "");
        return Name
    }
...
document.getElementById('load-conversations').addEventListener('click', async () => {
    ...
    // Check if there are existing unsaved messages and need confirmation
...
document.getElementById('clear-chat').addEventListener('click', () => {
    ...
    if (document.getElementById('chat-messages').innerHTML.length > 0 && currentConversationTouched == "true") {
        if (confirm('Are you sure you want to clear the current conversation?')) {
