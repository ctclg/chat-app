...
export class ConversationManager {
    ...
    async saveConversation(name, folder) {
        try {
            ...
            document.getElementById('current-chat').innerHTML = "Conversation: " + folder + " / " + name;
    ...
    async loadConversation(conversation) {
        ...
        try {
            this.chat.history = [];
            currentChatElement.innerHTML = `Conversation: ${conversation.folder} / ${conversation.name}`;
        ...
        return Object.entries(conversationsByFolder).map(([folder, convs]) => {
            const conversationItems = convs.map((conv) => {
                ...
                const conversationData = {
                    ...
                    published: conv.published || false
                };
                ...
                // Determine which button to add based on 'published' status
                let publishButtonHTML = '';
                if (conv.published === undefined || conv.published === false) { // Assuming undefined or false means not published
                    publishButtonHTML = `
                        <button class="publish-btn" title="Publish">
                            <i class="fas fa-cloud-upload-alt"></i>
                        </button>`;
                } else {
                    publishButtonHTML = `
                        <button class="manage-published-btn" title="Manage Published Conversation">
    ...
    setupConversationActions(dialog) {
        ...
        dialog.querySelectorAll('.publish-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                e.stopPropagation();
                const item = btn.closest('.conversation-item');
                const conversation = JSON.parse(item.dataset.conversation);

                if (confirm(`Are you sure you want to publish "${conversation.name}"?`)) {
                    await this.publishConversation(conversation.id);
                }
            });
        });

        // Setup unpublish functionality
        dialog.querySelectorAll('.manage-published-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                e.stopPropagation();
                const item = btn.closest('.conversation-item');
                const conversation = JSON.parse(item.dataset.conversation);

                if (confirm(`Are you sure you want to unpublish "${conversation.name}"?`)) {
                    await this.unpublishConversation(conversation.id);
                }
            });
        });
    ...
    async publishConversation(id) {
        try {
            ...
            const response = await fetch(`/api/publish-conversation/${id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ published: true, magiclink: "yyy" })
            });
            ...
            alert('Failed to publish conversation');
            throw error;
        }
    }

    async unpublishConversation(id) {
        try {
            const token = localStorage.getItem('token');
            if (!token) {
                throw new Error('No authentication token found');
            ...
            const response = await fetch(`/api/publish-conversation/${id}`, {
                ...
                body: JSON.stringify({ published: false, magiclink: "" })
            });
            ...
            alert('Failed to unpublish conversation');
            throw error;
        }
    }
